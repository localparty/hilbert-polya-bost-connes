...
**Theorem T3** (Self-adjointness). *$D_\infty$ is self-adjoint, $\mathrm{IsSelfAdjoint}\,D_\infty$, as a type-level field of $\mathrm{CCMGalerkinSpectralData}$ supplied by the gate inhabitant. The standard Stone–von Neumann criterion on the core $\bigcup_N P_N H_R$ provides the conceptual self-adjointness story; the current Lean scaffold realises this via `IsSelfAdjoint.smul` on the identity-scalar form of `dInftyCLM` pending non-scaffold gate inhabitation. The genuine unbounded modular Hamiltonian via continuous functional calculus on the closed positive $\Delta$ enters once a gate inhabitant is supplied.*
