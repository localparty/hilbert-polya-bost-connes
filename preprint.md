...
This paper develops a precise scaffold along that conjectural line. We construct, via Galerkin approximants over a chain of finite-dimensional projections, an unbounded operator $D_\infty$ on the GNS Hilbert space whose self-adjointness is required as a type-level field of the structured type $\mathrm{CCMGalerkinSpectralData}$ defined in §5 (i.e. supplied by a gate inhabitant, not derived within the present construction); we organise the Galerkin-side ingredients (uniform $H^1$ resolvent bounds, Anselone-Stummel collectively compact convergence, Bögli-Siegl-Tretter spectral exactness, Hurwitz uniform convergence of the regularised determinant) into that same structured type. The chain's terminal theorem is:
...
The cumulative picture: several distinct candidate substrates exist, none has produced a constructed self-adjoint operator inhabiting its programme's conditions, and each surfaces different irreducible hypotheses at the boundary of the open content. The present work's contribution is its specific reduction shape — a single concrete type-inhabitation problem against an operator-algebraic substrate that is closed independently and computer-verified — and its honesty about the conditionality of that reduction.
...
**Construction summary.** This section presents the structural form of the $D_\infty$ construction; the formalized proofs and the per-step Lean theorem statements live in the companion LeanBlueprint at \[hpbc-blueprint\], chapter `\Cref{ch:construction}` (`The compact resolvent and $D_\infty$ construction`). In particular, the Lean theorem `\Cref{thm:dinfty-clm-selfAdjoint}` records the structural form of self-adjointness as a type-level field of the Galerkin spectral-data gate (§5), and `\Cref{thm:ccm-complement-construction}` records the existential bundle of Galerkin spectral data + self-adjointness. The blueprint dependency graph at `https://localparty.github.io/hilbert-polya-bost-connes-lean/blueprint/dep_graph_document.html` makes the dependency structure between each step here and its Lean counterpart navigable in a single pass; the reader who wishes to inspect a specific step's formalized content should click through to the corresponding blueprint node.
...
**Spectral encoding summary.** This section presents the structural form of the spectrum identification; the formalized proofs and the per-axiom-and-theorem Lean statements live in the companion LeanBlueprint at \[hpbc-blueprint\], chapter `\Cref{ch:spectral-encoding}` (`The spectral encoding`). The Bögli no-spectral-pollution direction is recorded as `\Cref{thm:boegli}` (DERIVED theorem) reducing to the on-terminal atomic axiom `\Cref{ax:collectively-compact-uniform-bound}`; the Hurwitz zero-convergence direction is recorded as `\Cref{thm:hurwitz}` (DERIVED theorem) reducing to the on-terminal atomic axiom `\Cref{ax:rouche-zero-existence}`; the combined spectrum-zeros identity is recorded as `\Cref{thm:spectral-encoding}` (DERIVED from the gate). The blueprint dependency graph at `https://localparty.github.io/hilbert-polya-bost-connes-lean/blueprint/dep_graph_document.html` exposes the DERIVED-vs-atomic-axiom structure and the literature-citation discipline at each node.
...
The broader Hilbert-Pólya programme has been articulated and advanced over the decades by, among others, **M. Berry** and **J. Keating**, whose quantum-chaos / random-matrix connection has shaped contemporary understanding of zeta-zero spectral statistics, and **C. Deninger**, **M. Haran**, **N. Katz** and **P. Sarnak**, who have each pursued distinct foundational programmes. We also acknowledge **E. Bombieri**, whose contributions to analytic number theory and whose canonical expository statement of the Riemann Hypothesis \[Bom00\] have informed the present project's framing of the problem; the present work makes no claim regarding the Clay Mathematics Institute Millennium Prize, but cites Bom00 as the standard reference for the precise statement of the conjecture. The classical literature on $\zeta$ from **H. M. Edwards** and **E. C. Titchmarsh** has informed the analytic conventions used throughout.
...
**Bombieri, E.** *Problems of the Millennium: The Riemann Hypothesis.* Clay Mathematics Institute, 2000. (Standard expository statement of the conjecture; the present work does not address the Clay Millennium Prize.) <!-- \bibitem{Bom00} -->
